...
async function run() {
  ...
  // 1. Lire les positions IBKR
  console.log('Lecture des positions IBKR...');
  const positions = await readIbkrPositions();
  if (positions.length === 0) {
    console.error('Aucune position IBKR détectée. Le panneau Trading est-il ouvert ?');
    // Crée quand même un rapport d'alerte
    const md = `# Analyse Portefeuille — ${dateStr}\n\n> ${timeStr} ET\n\n⚠️ **Aucune position IBKR détectée** dans le panneau Trading de TradingView.\n\nVérifier que le panneau Trading est ouvert et connecté à Interactive Brokers.\n`;
    mkdirSync(REPORTS_DIR, { recursive: true });
    const alertPath = resolve(REPORTS_DIR, `portfolio_${dateStr}.md`);
    writeFileSync(alertPath, md, 'utf8');
    await sendReportEmail({
      subject: `[Portfolio V4] ⚠️ ${dateStr} — Aucune position détectée`,
      reportPath: alertPath,
      previewText: 'Le panneau Trading IBKR ne renvoie aucune position. Vérifier la connexion.',
    }).catch((e) => console.log(`Email erreur: ${e.message}`));
    process.exit(2);
  }

  console.log(`${positions.length} position(s) détectée(s):`);
  positions.forEach((p) => console.log(`  - ${toTradingViewSymbol(p)} ${p.side} ${p.qty} @ ${p.avgPrice}`));

  // 2. Analyser chaque position sur D + 4h
  const analyses = [];
  for (const pos of positions) {
    const tvSymbol = toTradingViewSymbol(pos);
    const indicatorCfg = getIndicatorFor(tvSymbol);
    console.log(`\nAnalyse ${tvSymbol} [lit ${indicatorCfg.name}]...`);
    try {
      ...
      const daily = perTf['D'];
      const h4 = perTf['240'];
      const recommendation = buildRecommendation(pos, daily, h4);

      analyses.push({ position: pos, tvSymbol, indicatorName: indicatorCfg.name, daily, h4, recommendation });
      console.log(`  → ${recommendation.action} (${recommendation.severity})`);
    } catch (err) {
      console.log(`  ERROR: ${err.message}`);
      analyses.push({ position: pos, tvSymbol, indicatorName: indicatorCfg.name, error: err.message });
    }
  }
  ...
  const md = renderMarkdown({ dateStr, timeStr, analyses });
  mkdirSync(REPORTS_DIR, { recursive: true });
  const filePath = resolve(REPORTS_DIR, `portfolio_${dateStr}.md`);
  ...
  try {
    ...
    execSync(`git commit -m "portfolio: analyse ${dateStr} (${analyses.length} positions)"`, {
      cwd: REPO_ROOT,
      stdio: 'pipe',
    });
  ...
  try {
    const critical = analyses.filter((a) => a.recommendation?.severity === 'high').length;
    const opportunities = analyses.filter((a) => a.recommendation?.severity === 'opportunity').length;
    const preview = `${analyses.length} position(s) analysée(s) · 🔴 ${critical} critique(s) · 🟢 ${opportunities} opportunité(s) d'ajout`;
    const subjectTag = critical > 0 ? '🔴' : opportunities > 0 ? '🟢' : '✅';
    const r = await sendReportEmail({
      subject: `[Portfolio V4] ${subjectTag} ${dateStr} — ${analyses.length} positions`,
      reportPath: filePath,
      previewText: preview,
    });
...
function renderMarkdown({ dateStr, timeStr, analyses }) {
  const L = [];
  L.push(`# Analyse Portefeuille IBKR — ${dateStr}`);
  L.push(`> Généré à ${timeStr} ET · ${analyses.length} position(s) · Multi-TF Daily + 4h · Indicateurs par ticker`);
  L.push('');

  // Warning si des indicateurs sont manquants
  const withErrors = analyses.filter((a) => a.daily?.error || a.h4?.error);
  if (withErrors.length > 0) {
    L.push('> ⚠️ **Indicateurs manquants sur le chart** — certaines analyses sont dégradées :');
    for (const a of withErrors) {
      const err = a.daily?.error || a.h4?.error;
      L.push(`> - ${a.tvSymbol}: ${err}`);
    }
    L.push('>');
    L.push('> Pour corriger : charge **Momentum V4** et **Gold Momentum Pro** sur ton chart TradingView actif.');
    L.push('');
  }

  // Résumé P/L
  const totalCost = analyses.reduce((s, a) => s + (a.position.avgPrice * a.position.qty || 0), 0);
  const totalValue = analyses.reduce((s, a) => s + (a.position.lastPrice * a.position.qty || 0), 0);
  const totalPnl = analyses.reduce((s, a) => s + (a.position.unrealizedPnl || 0), 0);
  const totalDaily = analyses.reduce((s, a) => s + (a.position.dailyPnl || 0), 0);

  L.push('## Résumé du compte');
  L.push('');
  L.push(`| Métrique | Valeur |`);
  L.push(`|---|---|`);
  L.push(`| Positions ouvertes | **${analyses.length}** |`);
  ...
  // Table résumé positions
  L.push('## Vue d\'ensemble');
  L.push('');
  L.push('| Ticker | Indic | Qty | Avg | Last | P/L CAD | P/L % | Signal D | Aligné 4h | Action |');
  L.push('|---|---|---|---|---|---|---|---|---|---|');
  for (const a of analyses) {
    const indicShort = a.indicatorName === 'Gold Momentum Pro' ? 'GoldPro' : 'V4';
    if (a.error) {
      L.push(`| **${a.tvSymbol}** | ${indicShort} | ${a.position.qty} | ${a.position.avgPrice} | - | - | - | - | - | ERROR |`);
      continue;
    }
    const p = a.position;
    const pnlPct = ((p.lastPrice - p.avgPrice) / p.avgPrice * 100).toFixed(1);
    const v4d = a.daily?.v4;
    let dSig = '-';
    if (v4d?.BRK > 0) dSig = 'BRK';
    else if (v4d?.PB > 0) dSig = 'PB';
    else if (v4d?.ADD > 0) dSig = 'ADD';
    else if (v4d?.EXIT > 0) dSig = 'EXIT';
    else if (v4d?.SELL > 0) dSig = 'SELL';
    else if (v4d?.WEAK > 0) dSig = 'WEAK';
    const h4Aligned = a.h4?.alignedBull ? '✅' : a.h4?.alignedBear ? '❌' : '⚪';
    const emoji = severityEmoji(a.recommendation.severity);
    L.push(`| **${a.tvSymbol}** | ${indicShort} | ${p.qty} | ${p.avgPrice} | ${p.lastPrice} | ${p.unrealizedPnl >= 0 ? '+' : ''}${p.unrealizedPnl} | ${pnlPct}% | ${dSig} | ${h4Aligned} | ${emoji} **${a.recommendation.action}** |`);
  }
  L.push('');

  // Détails par position
  L.push('---');
  L.push('## Analyse détaillée');
  L.push('');

  for (const a of analyses) {
    if (a.error) {
      L.push(`### ${a.tvSymbol} — ERREUR`);
      L.push('');
      L.push(`\`${a.error}\``);
      L.push('');
      continue;
    ...
    const pnlPct = ((p.lastPrice - p.avgPrice) / p.avgPrice * 100).toFixed(2);
    ...
    // Position
    L.push('**Position IBKR :**');
    L.push(`- Side: **${p.side}** · Qty: **${p.qty}**`);
    L.push(`- Prix d'entrée moyen: **${p.avgPrice}** · Dernier: **${p.lastPrice}** (${p.changePct})`);
    L.push(`- P/L latent: **${p.unrealizedPnl >= 0 ? '+' : ''}${p.unrealizedPnl} CAD** (${pnlPct}%)`);
    L.push(`- P/L du jour: ${p.dailyPnl >= 0 ? '+' : ''}${p.dailyPnl} CAD`);
    L.push('');

    // Analyse technique
    const isGold = d?.v4?._variant === 'gold-pro';
    L.push(`**Analyse technique (${a.indicatorName}) :**`);
    L.push('');
    if (isGold) {
    ...
    L.push('|---|---|');
    if (r.stop != null) {
      const stopLoss = (r.stop - p.avgPrice) * p.qty;
      L.push(`| Stop-loss suggéré | **${r.stop}** (si touché, P/L = ${stopLoss >= 0 ? '+' : ''}${round(stopLoss)} CAD) |`);
    }
  ...
  L.push('');
  L.push('- Respecter les stops suggérés (basés sur EMA Slow/Mid/Fast)');
  L.push('- Max 3% de risque par position');
  L.push('- Ne pas ajouter si extension > 8% au-dessus EMA Fast');
  L.push('- EXIT immédiat si signal SELL ou structure cassée (prix < EMA Slow)');
  L.push('');
  L.push('_Rapport généré par Portfolio Analyzer V4 · Source positions: TradingView Trading Panel / IBKR Live_');
  return L.join('\n');
}
